export default [{
    image: 'https://vote-quimby.s3.amazonaws.com/images/just-cant-fuck.jpg',
    title: 'Just Can\'t Fuck this Last Pint of Musiclar Famine unless the Death Flight arrives by Next Friday (Theoretically)',
    playList: [{
        title: 'Flight 19',
        url: '/mp3/just-cant-fuck/01.flight.mp3',
        artist: ['Vote Quimby']
    }, {
        title: 'Last Pint of Happiness',
        url: '/mp3/just-cant-fuck/02.pint.mp3',
        artist: ['Vote Quimby']
    }, {
        title: 'Fuck Recycling (We Should Play Elevator Music)',
        url: '/mp3/just-cant-fuck/03.recycling.mp3',
        artist: ['Vote Quimby']
    }, {
        title: 'Death of a Superhero',
        url: '/mp3/just-cant-fuck/04.superhero.mp3',
        artist: ['Vote Quimby']
    }, {
        title: 'Let\'s Go Out Next Friday',
        url: '/mp3/just-cant-fuck/05.friday.mp3',
        artist: ['Vote Quimby']
    }, {
        title: 'Just Can\'t Please Her',
        url: '/mp3/just-cant-fuck/06.pleaser.mp3',
        artist: ['Vote Quimby']
    }, {
        title: 'Famine 4 (Try the Veal)',
        url: '/mp3/just-cant-fuck/07.famine4.mp3',
        artist: ['Vote Quimby']
    }, {
        title: 'Theoretically',
        url: '/mp3/just-cant-fuck/08.theoretically.mp3',
        artist: ['Vote Quimby']
    }, {
        title: 'Musicular Bandslaughter',
        url: '/mp3/just-cant-fuck/09.musicular.mp3',
        artist: ['Vote Quimby']
    }]
},{
    image: 'https://vote-quimby.s3.amazonaws.com/images/talk-nerdy.jpg',
    title: 'Talk Nerdy to Me',
    playList: [{
        title: 'Traitors to the Cause',
        url: '/mp3/talk-nerdy/01.traitors.mp3',
        artist: ['Vote Quimby']
    }, {
        title: 'Limited Time Only',
        url: '/mp3/talk-nerdy/02.limited.mp3',
        artist: ['Vote Quimby']
    }, {
        title: 'Sci-Fi Nerd',
        url: '/mp3/talk-nerdy/03.sci-fi.mp3',
        artist: ['Vote Quimby']
    }, {
        title: 'Famine 3 (Famished)',
        url: '/mp3/talk-nerdy/04.famine3.mp3',
        artist: ['Vote Quimby']
    }, {
        title: 'To Be Continued',
        url: '/mp3/talk-nerdy/05.tbc.mp3',
        artist: ['Vote Quimby']
    }]
},{
    image: 'https://vote-quimby.s3.amazonaws.com/images/footloose.jpg',
    title: 'Footloose: The Revised Motion Picture Soundtrack',
    tracks: [{
        title: 'Footloose'
    }, {
        title: 'Let\'s Hear It for the Boy'
    }, {
        title: 'Almost Paradise'
    }, {
        title: 'Holding Out for a Hero'
    }, {
        title: 'Dancing in the Sheets'
    }, {
        title: 'I\'m Free (Heaven Helps the Man)'
    }, {
        title: 'Somebody\'s Eyes'
    }, {
        title: 'Never'
    }, {
        title: 'Waiting for a Girl Like You'
    }, {
        title: 'Dancing in the Sheets (Extended 12" Remix)'
    }],
    link: {
        label: 'Buy',
        href: 'https://store.cdbaby.com/cd/votequimby2'
    }
},{
    image: 'https://vote-quimby.s3.amazonaws.com/images/adequately.jpg',
    title: 'Adequately Prepared to Rock',
    tracks: [{
        title: 'Amalgam'
    }, {
        title: 'Secret of My Success'
    }, {
        title: 'Spin'
    }, {
        title: 'I\'m Not Bitter Song'
    }, {
        title: 'Test of Strength'
    }, {
        title: 'Years Ago'
    }, {
        title: 'Famine'
    }, {
        title: 'Plinko'
    }, {
        title: 'Enduro'
    }, {
        title: 'Advertisemento'
    }, {
        title: 'Famine II (The Famining)'
    }],
    link: {
        label: 'Buy',
        href: 'https://store.cdbaby.com/cd/votequimby'
    }
}];
